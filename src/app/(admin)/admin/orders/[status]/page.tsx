...
const Orders = ({
  ...
  const decodedCategorie = decodeURIComponent(`${params.status}`);
  const [orders, setOrders] = useState<any>([]);
  const router = useRouter();
  // fetch orders by status
  useEffect(() => {
   axiosInstance.post("/admin/get-orders", { status: decodedCategorie }).then((res) => {
          ...
          {orders.map((order:any,index:number) => {
            return (
              <tr key={order?._id}>
                ...
                <td className="text-center">
                  <button className="btn btn-primary" onClick={()=> handleShowDetails(order?._id)}>تفاصيل</button>
                </td>
